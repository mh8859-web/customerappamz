...
const Sidebar: React.FC<SidebarProps> = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const navItems = {
    Admin: [
      ...
      { to: '/admin/settings', icon: <SettingsIcon className="w-6 h-6" />, label: 'Settings' },
    ...
    Designer: [
      ...
      { to: '/designer/task-board', icon: <LayoutGridIcon className="w-6 h-6" />, label: 'Task Board' },
      { to: '/designer/my-calendar', icon: <CalendarIcon className="w-6 h-6" />, label: 'My Calendar' },
      { to: '/designer/team-calendar', icon: <UserGroupIcon className="w-6 h-6" />, label: 'Team Calendar' },
      { to: '/designer/daily-work', icon: <ClipboardIcon className="w-6 h-6" />, label: 'Daily Work' },
      { to: '/designer/my-attendance', icon: <ClockIcon className="w-6 h-6" />, label: 'My Attendance' },
      { to: '/designer/leave', icon: <CalendarIcon className="w-6 h-6" />, label: 'Leave' },
      { to: '/downloads', icon: <DownloadIcon className="w-6 h-6" />, label: 'Downloads' },
      { to: '/account', icon: <UserCircleIcon className="w-6 h-6" />, label: 'My Account' },
    ...
    Customer: [
      ...
      { to: '/customer/billing', icon: <CreditCardIcon className="w-6 h-6" />, label: 'Billing History' },
      { to: '/downloads', icon: <DownloadIcon className="w-6 h-6" />, label: 'Downloads' },
      { to: '/account', icon: <UserCircleIcon className="w-6 h-6" />, label: 'My Account' },
  ...
  return (
    <>
      ...
      />
      <aside 
        className={`fixed md:relative z-30 md:z-auto inset-y-0 left-0 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 w-72 bg-surface flex-shrink-0 transition-transform duration-300 ease-in-out border-r border-border-color/50 flex flex-col`}
      >
        ...
        <div className="px-4 py-4 border-t border-border-color/50">
          <button onClick={handleLogout} className={`${baseLinkClasses} ${inactiveLinkClasses} w-full`}>
