...
const AccountsDashboard: React.FC = () => {
    ...
    const handleProcessSalary = (designer: User, days: number) => {
        const amount = days * 1000; // Mock calculation
        if (window.confirm(`Process salary of ₹${amount} for ${designer.fullName} (${days} days)?`)) {
            alert(`Salary disbursed for ${designer.fullName}. System updated.`);
        }
    };
    ...
    return (
        <div className="space-y-10 pb-16">
            ...
            <div className="flex bg-slate-100/50 p-1.5 rounded-[24px] w-fit gap-2">
                <button onClick={() => setActiveSection('sentinel')} className={`px-8 py-3 rounded-[18px] text-[11px] font-black uppercase tracking-widest transition-all ${activeSection === 'sentinel' ? 'bg-white text-brand-blue shadow-card' : 'text-slate-400 hover:text-slate-600'}`}>10/40/45 Sentinel</button>
                ...
                <button onClick={() => setActiveSection('payroll')} className={`px-8 py-3 rounded-[18px] text-[11px] font-black uppercase tracking-widest transition-all ${activeSection === 'payroll' ? 'bg-white text-brand-blue shadow-card' : 'text-slate-400 hover:text-slate-600'}`}>Payroll Hub</button>
                <button onClick={() => setActiveSection('gp')} className={`px-8 py-3 rounded-[18px] text-[11px] font-black uppercase tracking-widest transition-all ${activeSection === 'gp' ? 'bg-white text-brand-blue shadow-card' : 'text-slate-400 hover:text-slate-600'}`}>Project GP%</button>
            </div>
                         ...
                         <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                <thead className="bg-slate-900 text-white">
                                    <tr>
                                        <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[3px]">Active Project</th>
                                        ...
                                        <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[3px] text-right">Push Alert</th>
                                    </tr>
                                </thead>
                ...
                {activeSection === 'payroll' && (
                    <Card className="luxury-glass p-0 overflow-hidden rounded-[40px] border-slate-100 shadow-premium">
                        <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Monthly Designer Payroll Cycle</h3>
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-[4px]">{new Date().toLocaleString('default', { month: 'long', year: 'numeric' })}</span>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                <thead className="bg-slate-50 text-slate-400 border-b border-slate-100">
                                    <tr>
                                        <th className="px-8 py-4 text-[10px] font-black uppercase tracking-[3px]">Member</th>
                                        <th className="px-8 py-4 text-[10px] font-black uppercase tracking-[3px] text-center">Attendance Logs</th>
                                        <th className="px-8 py-4 text-[10px] font-black uppercase tracking-[3px] text-center">Base Calculation</th>
                                        <th className="px-8 py-4 text-[10px] font-black uppercase tracking-[3px] text-right">Command</th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr key={d.id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-8 py-6 text-center">
                                                <div className="flex flex-col items-center">
                                                    ...
                                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Confirmed Days</span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-6 text-center">
                                                <div className="flex flex-col items-center">
                                                    <span className="text-sm font-display font-black text-slate-900 tabular-nums">₹{(d.daysWorked * 1000).toLocaleString()}</span>
                                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Gross Disburse</span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-6 text-right">
                                                <Button onClick={() => handleProcessSalary(d, d.daysWorked)} className="!rounded-full !px-8 !py-3 !bg-slate-900 !text-[10px] font-black uppercase tracking-widest shadow-button hover:scale-105 transition-transform">Process Payout</Button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                ...
                {activeSection === 'audit' && (
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                        <Card className="luxury-glass !p-10 rounded-[40px]">
                            <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-8">Site Vouchers (Audit)</h3>
                        ...
                        <Card className="luxury-glass !p-10 rounded-[40px]">
                            <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-8">Inventory Invoices (PO)</h3>
                            <div className="space-y-4">
                                {poQueue.map(p => (
